...
def try_question7():
    print(f"Testing {len(question7_ivs)} IVs for question 7...")

    for i, iv in enumerate(question7_ivs):
        print(f"Trying IV {i+1}/{len(question7_ivs)}: {iv}")
        ...
        try:
            ...
            # Answer first 6 questions (including the correct key we found)
            answers = [
                ...
                "7aeaef7351e88b7a",  # Question 6: Key yang digunakan untuk mengenkripsi file (correct!)
            ]
            ...
            # Now try question 7 with the current IV
            conn.recvuntil(f"No 7:".encode())
            conn.recvuntil(b"Answer: ")

            # Send the IV
            conn.sendline(iv.encode())

            # Wait for response
            response = conn.recvline().decode().strip()
            print(f"  Question 7: {response}")

            if "Correct" in response:
                print(f"\n🎉 SUCCESS! The correct IV for question 7 is: {iv}")
                return iv
            elif "Incorrect" in response:
                print(f"  IV {iv} is incorrect")
            else:
                print(f"  Unexpected response: {response}")

        except Exception as e:
            print(f"  Error with IV {iv}: {e}")
        finally:
            conn.close()

        print(f"  ---")

    print("\n❌ No correct IV found among the provided IVs")
    return None


if __name__ == "__main__":
    correct_iv = try_question7()
    if correct_iv:
        print(f"\n✅ Final answer for question 7: {correct_iv}")
    else:
        print("\n❌ Need to investigate further or try different IVs")
