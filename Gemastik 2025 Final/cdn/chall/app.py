...
def current_user():
    ...
    db = get_db()
    cur = db.execute(
        "SELECT id, username, role FROM users WHERE id = ?", (session["uid"],)
    )
    return cur.fetchone()
...
def _exiftool_text(path_on_disk: str) -> str:
    ...
    try:
        proc = subprocess.run(
            ["exiftool", "--", path_on_disk], capture_output=True, text=True, timeout=5
        )
...
def view_post(pid):
    ...
    for m in re.finditer(
        r"^\s*(File Name|Date Created)\s*:\s*(.*)$", metadata_full, flags=re.MULTILINE
    ):
    ...
    return render_template_string(
        tpl_src,
        post=post,
        file_name_val=file_name_val, date_created_val=date_created_val,
    )
