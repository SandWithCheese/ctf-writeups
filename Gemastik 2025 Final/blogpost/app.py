    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                logger.info(f"meta_file: {meta_file}")
            ...
            try:
                ...
                if os.path.exists(meta_file):
                    try:
                        ...
                        try:
                            with open(meta_file, "rb") as mf_src, open(
                                new_meta, "wb"
                            ) as mf_dst:
                ...
                if os.path.exists(new_meta):
                    try:
                        with open(
                            new_meta, "r", encoding="utf-8", errors="ignore"
                        ) as mf2:
            ...
            try:
                ...
                logger.info(f"metadata_text: {metadata_text}")
...
def uploaded_file(filename):
    ...
    db = get_db()
    cur = db.execute(
        "SELECT id FROM posts WHERE image_filename = ? AND author_id = ?",
...
def profile():
    ...
    with open(
        os.path.join(APP_DIR, "templates", "profile.html"), "r", encoding="utf-8"
    ) as fh:
